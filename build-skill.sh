#!/usr/bin/env bash
# build-skill.sh — Build self-contained SLV skill directories
#
# Usage: ./build-skill.sh <skill> [version]
#   skill:   validator | rpc | grpc-geyser
#   version: template version (default: detect from version.ts)
#
# Output: dist/skills/slv-<skill>/
#   ansible/   — skill-specific playbooks + resolved cmn/ dependencies
#   jinja/     — skill-specific Jinja templates + resolved cmn/ dependencies
#   SKILL.md   — skill documentation (if exists)
...
# Detect version from version.ts if not provided
if [[ -n "${2:-}" ]]; then
  VERSION="$2"
else
...
case "$SKILL" in
  ...
  rpc)
    ...
    ;;
  grpc-geyser)
    # gRPC Geyser uses mainnet-rpc playbooks (geyser_build, geyser_richat_build, etc.)
...
# Scan all copied playbooks for ../cmn/ imports
resolve_cmn_deps() {
  local seen=()
  local queue=()

  # First pass: find all cmn references from skill playbooks
  for dir in "${ANSIBLE_DIRS[@]}"; do
  ...
  # Process queue (handles second-level deps within cmn/)
  while [[ ${#queue[@]} -gt 0 ]]; do
    ...
    if [[ -f "$ANSIBLE_DIR/cmn/$current" ]]; then
      ...
      # Check for second-level cmn deps (relative, no ../)
  ...
  # Copy cmn/tasks/ if any playbook references it
...
SKILL_MD="$SCRIPT_DIR/skills/slv-$SKILL/SKILL.md"
if [[ -f "$SKILL_MD" ]]; then
  cp "$SKILL_MD" "$DIST_DIR/SKILL.md"
elif [[ -f "$SCRIPT_DIR/../../skills/slv-$SKILL/SKILL.md" ]]; then
  cp "$SCRIPT_DIR/../../skills/slv-$SKILL/SKILL.md" "$DIST_DIR/SKILL.md"
fi

# --- Summary ---
echo "✅ Built skill: slv-$SKILL (v$VERSION)"
